...
#: 톤이 갈리는 방향(불·공기)에서, 같은 원소의 운동이 어떻게 달라지는가.
#: (2026-09-07 확정 — "톤은 색이 아니라 운동으로 드러낸다")
#:
#: 원소색은 방향 4 × 강도 3 = 12색으로 이미 확정돼 있고(2026-08-19),
#: 톤은 그 체계에 들어가지 않는다. 그래서 이대로 두면 불·기쁨과 불·분노가
#: 색·배경이 완전히 같아 **화면에서 구분되지 않는다.** 색 체계를 다시
#: 열지 않고 톤을 드러내는 길이 운동이다.
#:
#: 두 갈래를 관통하는 축은 **운동의 일관성**이다.
#:
#:     밝은 톤(기쁨·희망)  향할 데가 있다 — 방향이 일정하고 흐름이 고르다
#:     어두운 톤(분노·불안) 향할 데가 없다 — 방향이 흩어지고 흐름이 끊긴다
#:
#: 물·흙은 톤 분기가 없으므로 여기에 없다. 두 원소의 valence는 고정값
#: 이라 운동을 가를 재료 자체가 없다.
TONE_MOTION = {
    ("fire", "joy"): "위로 곧게 피어오른다",
    ("fire", "anger"): "사방으로 터지며 흩어진다",
    ("air", "hope"): "한 방향으로 퍼져 나간다",
    ("air", "anxiety"): "흩어졌다 모이기를 반복한다",
}
